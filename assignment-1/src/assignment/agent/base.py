...
def parse_skill_file(path: Path) -> dict[str, str]:
    """Parse one SKILL.md into catalog metadata and full file content."""

    text = path.read_text()
    parts = text.lstrip().split("---", 2)
    if len(parts) < 3 or not parts[1].strip():
        raise ValueError(f"Malformed or missing YAML frontmatter in {path}")
    ...
    if not isinstance(name, str) or not name.strip():
        raise ValueError(f"YAML frontmatter in {path} is missing name")
    if not isinstance(description, str) or not description.strip():
        raise ValueError(f"YAML frontmatter in {path} is missing description")
...
def load_skill_directory(skills_path: Path) -> dict[str, dict[str, str]]:
    ...
    for child in sorted(skills_path.iterdir()):
        ...
        if not skill_file.is_file():
            raise ValueError(f"No SKILL.md found in {child}")
    ...
    def run(self) -> None:
        ...
        try:
            ...
            while not self.finished:
                ...
                tool_response = self.execute_tool_calls(tool_calls)
                self.messages.extend(tool_response)
